...
import Arel from '@/assets/foto-founders/arel.png';
import Faruq from '@/assets/foto-founders/faruq.png';
import Kunk from '@/assets/foto-founders/kunk.png';
import Mbak from '@/assets/foto-founders/mbak.png';
import Reciii from '@/assets/foto-founders/reciii.png';

interface LandingPageProps {
  onStart: () => void;
}

// Komponen Card Fitur (Tetap sama, sudah bagus)
const FeatureCard = ({ icon, title, description }: { icon: React.ReactElement, title: string, description: string }) => (
    <div className="bg-white/80 backdrop-blur-lg p-6 rounded-2xl shadow-lg border border-white/30 text-center transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 hover:bg-white h-full">
...
// Komponen Card Tim (DIPERBARUI - ukuran gambar)
    ...
    <div className="text-center group">
        <div className="relative inline-block">
            <img 
                src={imageUrl} 
                alt={name} 
                // DIPERBESAR: dari w-36 h-36 ke w-40 h-40
                className="w-40 h-40 rounded-full object-cover mx-auto shadow-lg border-4 border-white transition-all duration-300 group-hover:scale-110 group-hover:shadow-xl" 
            />
        </div>
...
// Komponen "Step" (Tetap sama, sudah bagus)
const StepCard = ({ number, icon, title, description }: { number: string, icon: React.ReactElement, title: string, description: string }) => (
  <div className="relative bg-white/80 backdrop-blur-lg p-6 rounded-2xl shadow-xl border border-white/50 transition-all duration-300 hover:-translate-y-1 h-full">
...
// BARU: Data Tim untuk di-loop
const teamMembers = [
    { name: "Reqi Jumantara Hapid", role: "Project Manager", imageUrl: Reciii },
    { name: "Wardah Aulia Azzahra", role: "Data & Culture Researcher", imageUrl: Mbak },
    { name: "Umar Faruq Robbany", role: "Lead Developer", imageUrl: Faruq },
    { name: "Farrel Zandra", role: "AI Engineer", imageUrl: Arel },
    { name: "Banteng Harisantoso", role: "UI/UX Designer", imageUrl: Kunk }
];
...
export const LandingPage: React.FC<LandingPageProps> = ({ onStart }) => {
    return (
        <div className="bg-stone-50 text-stone-800 antialiased">
            ...
            {/* Hero Section (Sudah bagus, tidak diubah) */}
            ...
            {/* About Section (DIPERBARUI - Video diganti <img>) */}
            <section id="about" className="py-24 px-4 bg-white">
                <div className="container mx-auto">
                    <div className="grid md:grid-cols-2 gap-16 items-center">
                        ...
                        {/* Kolom Gambar (GANTI video dengan <img>) */}
            ...
            {/* How It Works Section (DIPERBARUI - Animasi staggered) */}
            <section id="how-it-works" className="py-24 px-4 bg-stone-100/70" style={{backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23d3c5b4' fill-opacity='0.1'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")`}}>
                <div className="container mx-auto text-center max-w-5xl">
                    ...
                    <div className="grid md:grid-cols-3 gap-8">
                        {/* DITAMBAH: wrapper div untuk staggered animation delay */}
            ...
            {/* Features Section (DIPERBARUI - Animasi staggered) */}
            ...
            {/* Team Section (DIPERBARUI - Refaktor ke .map() & animasi) */}
            <section id="team" className="py-24 px-4 bg-stone-100/70" style={{backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23d3c5b4' fill-opacity='0.1'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")`}}>
                <div className="container mx-auto text-center">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-10">
                        {/* REFAKTOR: Menggunakan loop .map() agar lebih bersih */}
                        {teamMembers.map((member, index) => (
                            <div key={member.name} className="animate-fade-in-up" style={{ animationDelay: `${index * 100}ms` }}>
                                <TeamMemberCard 
                                    name={member.name}
                                    role={member.role}
            ...
            {/* Footer (DIPERBARUI - Layout lebih baik & ada link navigasi) */}
            <Footer onNavigate={onStart} />
            
            {/* Animasi CSS (Tetap sama) */}
            <style>{`
                html {
