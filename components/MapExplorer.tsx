    ...
    if (mapContainerRef.current && (window as any).L) {
      ...
      clothingItems.forEach(item => {
        ...
        const popupContent = `
          <div class="p-1">
            ...
            <div class="p-2 pt-0">
                ...
                <button id="try-on-${item.id}" class="w-full bg-amber-800 text-white font-bold py-2 px-4 rounded-lg hover:bg-amber-900 transition-all text-sm shadow-md hover:shadow-lg">
                Coba Pakaian Ini
                </button>
            </div>
          </div>
        ...
        marker.on('popupopen', () => {
          ...
          if (button) {
            button.onclick = () => onSelectTribe(item);
          }
        });
      });
    }
  ...
  return (
    <div>
        <div className="text-center mb-8 md:mb-12">
            <h1 className="text-4xl md:text-5xl font-bold text-stone-900 font-display">Peta Budaya WastraNusa</h1>
            <p className="mt-4 text-lg md:text-xl text-stone-600 max-w-3xl mx-auto">
                Jelajahi kekayaan Pakaian Adat Nusantara dan coba langsung secara virtual. Klik penanda pada peta untuk memulai.
            </p>
        </div>
        <div ref={mapContainerRef} className="w-full h-[60vh] md:h-[70vh] rounded-2xl shadow-xl z-10" />
    </div>
  );
};